...
MainWindow::MainWindow() : QMainWindow(nullptr), running(false) {
...
void MainWindow::setupStyle() {
    ...
    for (auto& i : { startButton, pauseButton, stopButton }) i->setProperty("controlButton", true);
}
...
void MainWindow::counting() {
    if (timer->isChecked()) {   // timer
        if (secLabel->text().toInt() != 0 || minLabel->text().toInt() != 0 ||
...
void MainWindow::takeSec() {
    ...
    if (secLabel->text().toInt() == 0 && minLabel->text().toInt() == 0 &&
        hourLabel->text().toInt() == 0 && running) {
        emit timeout();
        running = false;
    }
}
